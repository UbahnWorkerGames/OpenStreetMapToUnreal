...
function buildAreaFeatureTooltip(feature) {
  ...
  const PRIORITY_KEYS = [
    "highway", "building", "natural", "waterway", "railway", "landuse",
    "amenity", "shop", "tourism", "leisure", "barrier", "man_made", "power", "aeroway",
    "surface", "smoothness",
    "height", "building:height", "building:levels", "roof:shape", "roof:colour",
    "building:colour", "building:material",
    "species", "genus", "leaf_type", "leaf_cycle", "denotation",
  ...
  for (const [k, v] of Object.entries(tags)) {
    ...
    const entry = { key: k, value: String(v) };
...
async function captureMapImageForGroundPlane() {
  ...
  try {
    ...
    // Rotate 90° CW (270° CCW) to align with UE plane UV mapping
    const rotated = document.createElement("canvas");
    rotated.width = h;
    rotated.height = w;
    const rotCtx = rotated.getContext("2d");
    rotCtx.setTransform(0, -1, 1, 0, 0, w);
    rotCtx.drawImage(resized, 0, 0, w, h, 0, 0, w, h);
